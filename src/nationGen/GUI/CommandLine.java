  ...
  public CommandLine(String[] args) throws Exception {
    ...
    for (String str : args) {
      ...
      if (str.equals("-seed")) {
        ...
        if (nextseed) seed = Long.parseLong(str);
        if (nextamount) amount = Integer.parseInt(str);
        if (nextsettings) settings = Integer.parseInt(str);
        if (nextnseed) seeds.add(Long.parseLong(str));
      } else if (nextname) {
        if (modname == null) modname = str;
        else modname = modname + " " + str;
    ...
    if (seeds.size() > 0) ng.generate(seeds);
    else if (seed != null) ng.generate(amount, seed);
    else ng.generate(amount);
  }
}
