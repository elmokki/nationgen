...
public class SpriteGen extends JFrame {
	...
	private class ImageSaverPanel extends JPanel {
		...
		ImageSaverPanel() {
			...
			JPanel displayCenterer = new JPanel(new TableLayout(1, 1, 0, 0, TableLayout.Alignment.SOUTH));
			displayCenterer.add(display);
			add(displayCenterer);
			
			controls = new JPanel(new TableLayout(2, 1, 8, 8, TableLayout.Alignment.NORTH));
			JPanel radios = new JPanel(new TableLayout(1, 2, 8, 8, TableLayout.Alignment.CENTER));
		...
		public void saveImage() {
			if (chooser.getSelectedFile() == null || !chooser.getSelectedFile().getName().endsWith(fileType)) {
	...
	private class UnitSpriteMixer {
		...
		private class SlotControls {
			...
			public SlotControls(String slot, List<Item> itemsToAdd) {
				...
				int maxImageWidth = itemsToAdd.stream()
					.filter(item -> !"".equals(item.sprite))
					.mapToInt(item -> FileUtil.readImage(item.sprite).getWidth()).max().orElse(0);
				items.setPreferredSize(new Dimension(d.width + maxImageWidth, d.height));
				
				offsetx.setPreferredSize(new Dimension(50, offsetx.getPreferredSize().height));
				...
				offsety.setPreferredSize(new Dimension(50, offsety.getPreferredSize().height));
		...
		public final JPanel content = new JPanel(new TableLayout(1, 2, 8, 8, TableLayout.Alignment.NORTH_WEST));
		...
		UnitSpriteMixer() {
			...
			// LEFT SIDE:
			// Colors
			colorChooser.getSelectionModel().addChangeListener(a -> {
			...
			colorChooser.getSelectionModel().addChangeListener(c -> colorPreviewPanel.setBackground(colorChooser.getColor()));
			
			// Renders
			JPanel renders = new JPanel(new TableLayout(1, 0, 8, 8, TableLayout.Alignment.NORTH));
			renders.add(drawpanel);
			renders.add(drawPanel2x);
			
			JPanel leftPanel = new JPanel(new TableLayout(3, 1, 8, 8, TableLayout.Alignment.NORTH_WEST));
			...
			// RIGHT SIDE:
			// race/pose
			JPanel raceposepanel = new JPanel(new TableLayout(1, 0, 8, 8, TableLayout.Alignment.NORTH_EAST));
			...
			JPanel rightPanel = new JPanel(new TableLayout(0, 1, 8, 16, TableLayout.Alignment.NORTH_WEST));
		...
		private void updateItems() {
			...
			itempanel.setLayout(new TableLayout(1 + p.getListOfSlots().size(), 4, 8, 8,
	...
	private class FlagSpriteMixer {
		class FlagPart {
			...
			FlagPart(String name, List<Flag> flags) {
				...
				color.addActionListener(e -> colorChooser.getSelectionModel().setSelectedColor(getColor()));
				colorButtonGroup.add(color);
				
				applyColor.addActionListener(e -> setColor(colorChooser.getSelectionModel().getSelectedColor()));
		...
		FlagSpriteMixer() {
			...
			JPanel leftPanel = new JPanel(new TableLayout(0, 1, 8, 8, TableLayout.Alignment.NORTH_WEST));
			leftPanel.add(colorChooser);
			leftPanel.add(renders);
			
			JPanel partsTable = new JPanel(new TableLayout(5, 4, 8, 8, TableLayout.Alignment.NORTH_WEST));
		...
		public void drawFlag() {
			...
			BufferedImage tga = new BufferedImage(128, 128, BufferedImage.TYPE_INT_ARGB);
