...
public class TableLayout implements LayoutManager2, java.io.Serializable {
  ...
  private class TableDimensions {
    ...
    TableDimensions(
      Container parent,
      Function<Component, Dimension> whichSize
    ) {
    ...
    Dimension getTotal() {
      return new Dimension(
        insets.left + insets.right + IntStream.of(w).sum() + (ncols - 1) * hgap,
  ...
  public void layoutContainer(Container parent) {
    synchronized (parent.getTreeLock()) {
      TableDimensions d = new TableDimensions(
        parent,
        Component::getPreferredSize
      );
      ...
      int extraTotalWidthAvailable =
        widthWOInsets - (IntStream.of(d.w).sum() + totalGapsWidth);
      int extraLeftWidthAvailable = Math.round(
        extraTotalWidthAvailable * alignment.x
      );
      int extraRightWidthAvailable =
        extraTotalWidthAvailable - extraLeftWidthAvailable;

      int totalGapsHeight = (d.nrows - 1) * vgap;
      int heightWOInsets =
        parent.getHeight() - (d.insets.top + d.insets.bottom);
      int extraTotalHeightAvailable =
        heightWOInsets - (IntStream.of(d.h).sum() + totalGapsHeight);
      int extraTopHeightAvailable = Math.round(
        extraTotalHeightAvailable * alignment.y
      );
      ...
      if (ltr) {
        for (
          int c = 0, x = d.insets.left + extraLeftWidthAvailable;
          c < d.ncols;
          x += d.w[c] + hgap, c++
        ) {
          for (
            int r = 0, y = d.insets.top + extraTopHeightAvailable;
            r < d.nrows;
            y += d.h[r] + vgap, r++
          ) {
            int i = r * d.ncols + c;
        ...
        for (
          int c = 0, x =
            (parent.getWidth() - d.insets.right - d.w[c]) -
            extraRightWidthAvailable;
          c < d.ncols;
          c++, x -= d.w[c] + hgap
        ) {
          for (
