...
public class PriestNamer extends MageNamer {
  ...
  public void execute(Nation n) {
    ...
    for (int i = 2; i >= 0; i--) {
      ...
      if (i < 2 && all.get(i + 1).size() > 0) {
        ...
        if (i < 2) this.generateNewNames(all.get(i), 3, prefixrank, suffix);
        else this.generateNewNames(all.get(i), (i + 1), prefixrank, suffix);
      }
    }
  }
  ...
  protected void loadNameData() {
    ...
    n.races
      .get(0)
      .tags.getAllStrings("priestrankedprefixes")
