...
public class MageDescriber {
  ...
  public static String getCommonNoun(Nation n) {
    ...
    String derp = "";
    if (noun.prefix != null) derp = noun.prefix.toString();
    if (noun.prefixprefix != null) derp = noun.prefixprefix.toString() +
    " " +
    derp;
    if (noun.suffix != null) derp = noun.suffix.toString();
    if (noun.suffixprefix != null && noun.prefix == null) derp =
      noun.suffixprefix.toString() + " " + derp;

    return derp.trim();
  }

  public static Name getCommonName(Nation n) {
    List<Unit> primaryMages = n
      .selectCommanders()
    ...
    if (primaryMages.size() > 0) {
      ...
      if (
        common.suffix == null && common.suffixprefix != null
  ...
  private static void updateCommonNameFromUnit(Unit u, Name common) {
    if (
      u.name.prefix == null || !u.name.prefix.equals(common.prefix)
    ) common.prefix = null;
    if (u.name.type == null || !u.name.type.equals(common.type)) common.setType(
      "mage"
    );
    if (
      u.name.suffixprefix == null ||
      !u.name.suffixprefix.equals(common.suffixprefix)
    ) common.suffixprefix = null;
    if (
      u.name.suffix == null || !u.name.suffix.equals(common.suffix)
    ) common.suffix = null;
  }
}
