...
public class Race extends Filter {
  ...
  public void addCommand(Command c) {
    for (int i = 0; i < unitcommands.size(); i++) if (
      unitcommands.get(i).command.equals(c.command)
    ) {
  ...
  public boolean hasSpecialRole(String role, boolean sacred) {
    String str = "elite";
    boolean allok = false;
    if (sacred && this.tags.containsName("all_troops_sacred")) allok = true;
    else if (!sacred && this.tags.containsName("all_troops_elite")) allok =
      true;

    if (sacred) str = "sacred";
    for (Pose p : poses) {
      for (String role2 : p.roles) if (
        role2.toLowerCase().equals((str + " " + role).toLowerCase()) ||
        (role2.equals(role) && allok) ||
        (role2.equals(role) && p.roles.contains(str))
      ) return true;
    }
    return false;
  }
  ...
  public void handleOwnCommand(Command command) {
    switch (command.command) {
      ...
      case "#specialcommand":
        ...
        break;
      case "#pose": {
        List<Pose> set = nationGen
          .getAssets()
          .poses.get(command.args.get(0).get());
        if (set == null) {
          throw new IllegalArgumentException(
            "Pose set " + command.args.get(0).get() + " was not found."
          );
        } else {
  ...
  public ItemSet getItems(String slot, String role) {
    ...
    for (Pose p : poses) if (p.roles.contains(role)) {
      if (p.getItems(slot) != null) items.addAll(p.getItems(slot));
    }
  ...
  protected void finish() {
    if (visiblename == null) visiblename = name;
  }
