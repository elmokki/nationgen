...
public class Entity {
        ...
        if (strLine.trim().toLowerCase().startsWith("#new")) {
          if (instance != null) {
            throw new IllegalStateException(
              "The previous definition must be closed with #end before starting another with #new"
            );
          }
          try {
            ...
            throw new IllegalStateException(
              "Error initializing a new instance of " + c.getCanonicalName(),
              e
            );
          }
  ...
  public void handleOwnCommand(Command command) {
    if (command.command.equals("#id") || command.command.equals("#name")) {
      ...
      command.command.equals("#basechance") || command.command.equals("#chance")
    ) {
