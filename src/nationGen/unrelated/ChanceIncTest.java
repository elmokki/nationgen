...
public class ChanceIncTest {
  ...
  public ChanceIncTest() {
    ...
    System.out.println(
      "7: " + checkChanceInc("true and not false") + " / true"
    );
    System.out.println(
      "8: " + checkChanceInc("not false and true") + " / true"
    );
  }
  ...
  public List<String> partitionChanceInc(String str) {
    ...
    if (con.length() > 0) chanceincs.add(con);
  ...
  private boolean check(String s) {
    ...
    if (s.startsWith("not ")) s = s.substring(4);

    boolean ok = false;
    if (s.equals("true")) ok = true;
