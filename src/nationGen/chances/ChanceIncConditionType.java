...
public enum ChanceIncConditionType {
	...
	SHAPE("shape") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> d.n.secondshapes.stream().anyMatch(u -> u.thisForm.name.equals(shape)) != not;
		}
	},
	...
	SPELLS("spells") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> d.n.spells.stream().anyMatch(s -> s.name.equalsIgnoreCase(name)) != not;
		}
	},
	...
	HAS_THEME("hastheme") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> d.n.races.size() > 0 && d.n.races.get(0).themefilters.stream().anyMatch(t -> t.name.equals(theme)) != not;
		}
	},
	...
	ANY_THEME("anytheme") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> d.n.races.size() > 0
					&& (d.n.races.get(0).themefilters.stream().anyMatch(t -> t.themes.contains(theme))
						|| d.n.races.get(0).themes.contains(theme)) != not;
		}
	},
	...
	HAS_THEME_THEME("hasthemetheme") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> d.n.races.size() > 0 && d.n.races.get(0).themefilters.stream().anyMatch(t -> t.themes.contains(theme)) != not;
		}
	},
	...
	SECONDARY_THEME_THEME("secondarythemetheme") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> d.n.races.size() > 1 && d.n.races.get(1).themefilters.stream().anyMatch(t -> t.themes.contains(theme)) != not;
		}
	},
	...
	SECONDARY_RACE_THEME("secondaryracetheme") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> d.n.races.size() > 1 && d.n.races.get(1).themefilters.stream().anyMatch(t -> t.name.equals(theme)) != not;
		}
	},
	...
	MAGIC_PRIORITY("magicpriority") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			MagicPath path = args.nextMagicPath();
			boolean not = args.nextOptionalFlag("not") ^ args.nextOptionalFlag("below");
			double target = args.nextDecimal();
			
			return d -> {
				double chance = 1;
				for(Args priorityArgs : Generic.getAllNationTags(d.n).getAllArgs("magicpriority"))
				{
	...
	PERCENTAGE_OF_RACE("percentageofrace") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> {
				Optional<Race> r = d.n.races.stream().filter(r2 -> race.equalsIgnoreCase(r2.name)).findFirst();
				
				return (r.isPresent() && d.n.percentageOfRace(r.get()) >= percent) != not;
			};
		}
	},
	...
	PRIMARY_RACE("primaryrace") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> (d.n.races.size() > 0 && d.n.races.get(0).name.equalsIgnoreCase(race)) != not;
		}
	},
	...
	SECONDARY_RACE("secondaryrace") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> (d.n.races.size() > 1 && d.n.races.get(1).name.equalsIgnoreCase(race)) != not;
		}
	},
	...
	NATION_TAG("nationtag") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> (d.n.nationthemes.stream().anyMatch(f -> f.tags.containsTag(tag))
					|| d.n.races.size() > 0 && d.n.races.get(0).tags.containsTag(tag)) != not;
		}
	},
	...
	RACE_TAG("racetag") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			boolean not = args.nextOptionalFlag("not");
			Command tag = args.nextCommand();
			return d -> (d.n.races.size() > 0 && d.n.races.get(0).tags.containsTag(tag) != not)
	...
	POSE("pose") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> (d.u.pose.roles.contains(pose)
					|| d.u.pose.roles.contains("elite " + pose)
					|| d.u.pose.roles.contains("sacred " + pose)) != not;
		}
	},
	...
	PERSONAL_SHAPE("personalshape") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> d.u != null
				&& d.n.secondshapes.stream().anyMatch(u -> u.thisForm.name.equals(shape) && u.otherForm.equals(d.u)) != not;
		}
	},
	...
	FILTER("filter") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> d.u.appliedFilters.stream().anyMatch(f -> f.name.equals(filter)) != not;
		}
	},
	...
	UNIT_THEME("unittheme") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> (d.u.race.themes.contains(theme) || d.u.pose.themes.contains(theme)
					|| d.u.race.themefilters.stream().anyMatch(f -> f.themes.contains(theme))
					|| d.u.appliedFilters.stream().anyMatch(f -> f.themes.contains(theme))) != not;
		}
	},
	...
	RACE_THEME("racetheme") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d ->	(d.race.themes.contains(theme)
					|| d.race.themefilters.stream().anyMatch(f -> f.name.equals(theme) || f.themes.contains(theme))) != not;
		}
	},
				...
				if (i != null) {
					...
					if ((!armor || i.armor) && (!weapon || !i.armor)) {
						if (id.isEmpty() || i.id.equals(id.get())) {
							...
							contains = ci.olditem != null && ci.olditem.id != null && ci.olditem.id.equals(id.get())
	...
	SLOT_NAME("slotname") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> {
				...
				Item i = d.u.getSlot(slot);
				if (i != null) {
					return ((!armor || i.armor) && (!weapon || !i.armor)
							&& (name.isEmpty() || i.name.equals(name.get()))) != not;
				}
				return false;
			};
		}
	...
	SLOT_TAG_VALUE("slottagvalue") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> {
				...
				Item i = d.u.getSlot(slot);
				return i != null && i.tags.getValue(tagName).stream().anyMatch(tagValue::equals) != not;
			};
		}
	},
	...
	POSE_ITEM_THEME("poseitemtheme") {
		...
		Condition<ChanceIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> Optional.ofNullable(d.u.pose.getItems(slot))
					.map(set -> set.stream().filter(Objects::nonNull).anyMatch(i -> i.themes.contains(theme)))
					.orElse(false) != not;
		}
	},
			...
			return d -> {
				...
				switch (comparison) {
					...
					case EQUAL:
						return picks.get(path1) == picks.get(path2) != not;
					default:
						throw new IllegalArgumentException("comparemagic doesn't know how to compare with " + comparison);
				}
			};
		}
	},
	...
	static {
		...
		for (ChanceIncConditionType type : values()) {
			if (temp.put(type.name, type) != null) {
				throw new IllegalArgumentException(String.format("ConditionType %s has duplicate name '%s'", type, type.name));
			}
			for (String name : type.alternateNames) {
				if (temp.put(name, type) != null) {
					throw new IllegalArgumentException(String.format("ConditionType %s has duplicate name '%s'", type, name));
				}
			}
		}
