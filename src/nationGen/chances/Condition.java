...
public interface Condition<T> {
	...
	static <T> Condition<T> from(Args args, Function<Args, Condition<T>> conditionCreator) {
		...
		for (Arg arg : args) {
			if (arg.isParenthesis()) {
				if (condition != null && joiner == null) {
					throw new IllegalArgumentException(String.format(
							"Unexpected parenthesis %s; expected [AND] or [OR] in args %s", arg, args));
				}
				Condition<T> cond = Condition.from(arg.getParenthesis(), conditionCreator);
				if (condition == null) {
					condition = cond;
				} else {
			...
			} else if (arg.get().equalsIgnoreCase("AND") || arg.get().equalsIgnoreCase("OR")) {
				if (joiner != null) {
					...
					condition = joiner.join(condition, conditionCreator.apply(runningArgs));
					runningArgs.clear();
				} else if (condition == null) {
					if (runningArgs.isEmpty()) {
						throw new IllegalArgumentException(String.format(
								"Unexpected condition joiner [%s]; expected condition in args %s.", arg, args));
					}
				...
				if (condition != null && joiner == null) {
					throw new IllegalArgumentException(String.format(
							"Missing condition joiner after parenthesis in args %s", args));
				}
				runningArgs.add(arg);
			}
		}
		if (!runningArgs.isEmpty()) {
			...
			throw new IllegalArgumentException(String.format(
					"Dangling condition joiner [%s] at end of args %s", joiner, args));
		}
		if (condition == null) {
