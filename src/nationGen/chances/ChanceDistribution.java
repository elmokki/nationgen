...
public class ChanceDistribution<E> {
	...
	public void modifyChance(E f, Arg mod) {
		this.chances.compute(f, (entity, chance) -> mod.applyModTo(chance == null ? baseChance(f) : chance));
	}
	...
	public int countPossible() {
		return (int) this.chances.values().stream().filter(chance -> chance > 0).count();
	}
	
	public List<E> getPossible() {
		return this.chances.entrySet().stream()
				.filter(e -> e.getValue() > 0)
	...
	public E getRandom(Random r) {
		...
		if(item == null)
			throw new IllegalStateException("ITEM PICKING FAILURE. Strange, eh?");
