...
public enum ThemeIncConditionType {
	...
	THIS_ITEM_SLOT_TAG("thisitemslottag") {
		...
		Condition<ThemeIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> d.f instanceof Item && ((Item) d.f).slot.equals(slot)
		...
		Condition<ThemeIncData> parseConditionArguments(ArgParser args) {
			...
			return d -> {
				if (d.f instanceof Item) {
					Item i = (Item) d.f;
					if (!i.armor) {
						boolean slash = d.nationGen.weapondb.GetInteger(i.id, "dt_slash", 0) == 1;
						boolean blunt = d.nationGen.weapondb.GetInteger(i.id, "dt_blunt", 0) == 1;
						boolean pierce = d.nationGen.weapondb.GetInteger(i.id, "dt_pierce", 0) == 1;
