...
public class Tags {
	...
	public void add(List<String> tag) {
		...
		Args args = tag.subList(1, tag.size()).stream().map(Arg::new).collect(Collectors.toCollection(Args::new));
		add(tag.get(0), args);
	}
	...
	public void addArgs(String name, Object... values) {
		computeArgsList(name).add(Arrays.stream(values).map(Object::toString).map(Arg::new).collect(Collectors.toCollection(Args::new)));
	}
	...
	public boolean removeAll(String name, Collection<Args> argses) {
		return this.tagValues.getOrDefault(name, new ArrayList<>()).removeAll(argses);
	}
	...
	public List<Double> getAllDoubles(String name) {
		return streamAllValues(name).map(Arg::getDouble).collect(Collectors.toList());
	}
	
	public List<Command> getAllCommands(String name) {
		return streamAllValues(name).map(Arg::getCommand).collect(Collectors.toList());
	}
	
	public Optional<Args> getArgs(String name) {
		return this.tagValues.containsKey(name) ? this.tagValues.get(name).stream().findFirst() : Optional.empty();
	}
	...
	public boolean contains(String name, Object value) {
		...
		return this.tagValues.containsKey(name)
			&& this.tagValues.get(name).stream().flatMap(List::stream).map(Arg::get).anyMatch(stringValue::equals);
	}
	...
	public boolean isEmpty() {
		return this.tagValues.isEmpty() || this.tagValues.values().stream().allMatch(List::isEmpty);
	}
	
	public String toString() {
		return this.tagValues.entrySet().stream()
