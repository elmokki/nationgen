...
public class FlagGen {
  ...
  public FlagGen(Nation n, NationGenAssets assets) {
    ...
    n.races
      .get(0)
      .tags.getAllStrings("flagtops")
      .forEach(set -> top.addAll(assets.flagparts.get(set)));
    n.races
      .get(0)
      .tags.getAllStrings("flagborders")
      .forEach(set -> border.addAll(assets.flagparts.get(set)));
    n.races
      .get(0)
      .tags.getAllStrings("flagbases")
      .forEach(set -> bases.addAll(assets.flagparts.get(set)));
    n.races
      .get(0)
      .tags.getAllStrings("flagpoles")
      .forEach(set -> poles.addAll(assets.flagparts.get(set)));

    if (top.size() == 0) top.addAll(assets.flagparts.get("topicons"));
    if (poles.size() == 0) poles.addAll(assets.flagparts.get("poles"));
    if (border.size() == 0) border.addAll(assets.flagparts.get("borders"));
    if (bases.size() == 0) bases.addAll(assets.flagparts.get("baseflags"));
  }
  ...
  private boolean drawIcon(
    ...
    if (stuff == border) isBorder = true;

    Graphics g = image.getGraphics();

    List<Flag> possible = new ArrayList<>();
    for (Flag f : stuff) {
      if (f.allowed.size() == 0 || f.allowed.contains(basename)) possible.add(
        f
      );
    }
    ...
    if (n.random.nextDouble() > 0.5 && isBorder) {
      int red = Math.max(
        0,
        Math.min(col.getRed() - 30, n.colors[0].getRed() - 30)
      );
      int green = Math.max(
        0,
        Math.min(col.getGreen() - 30, n.colors[0].getGreen() - 30)
      );
      int blue = Math.max(
        0,
        Math.min(col.getBlue() - 30, n.colors[0].getBlue() - 30)
      );
      col = new Color(red, green, blue);
    } else if (n.random.nextDouble() > 0.75) col = n.colors[2];
