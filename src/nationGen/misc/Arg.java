...
public class Arg {
	...
	public int getInt() {
		if (this.integer == null) {
			...
			} else if (getOperator().filter(o -> o != Operator.SUBTRACT).isPresent()) {
	...
	public Args getParenthesis() {
		return this.parenthesis == null ? null
				: this.parenthesis.copy(); // defensive copy to prevent this object's copy being modified
	}
		...
		if (getOperator().isPresent()) {
			switch (getOperator().get()) {
				...
				case DIVIDE:
					return value / getDouble();
				default:
					throw new UnsupportedOperationException("Unknown how to apply mod operator [" + getOperator().get() + "].");
			}
		} else {
			return value + getDouble();
		}
	...
	public Arg applyModToNothing() {
		...
		if (operator.isPresent()) {
			...
			} else if (operator.get() == Operator.MULTIPLY || operator.get() == Operator.DIVIDE) {
				return new Arg(0);
			}
		}
	...
	public String unparse() {
		...
		} else if (Generic.containsSpace(this.string) || this.string.startsWith("\"") || this.string.startsWith("'")) {
