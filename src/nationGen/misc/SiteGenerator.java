...
public class SiteGenerator {
  ...
  public static void generateSites(Nation n, NationGenAssets assets) {
    ...
    List<Filter> filters = ChanceIncHandler.retrieveFilters(
      "sitefeatures",
      "default_sitefeatures",
      assets.miscdef,
      null,
      n.races.get(0)
    );
    Filter f = chandler.getRandom(filters);
    if (!f.name.equals("nothing")) stuff++;
        ...
        if (added < max && gems > 0) {
          ...
          if (!done) {
            ...
            for (MagicPath i : MagicPath.values()) {
              if (totalgems.get(i) > 0) possibles.add(i);
            }
  ...
  private static List<Unit> getRelatives(List<Unit> list, Unit u) {
    List<String> identifiers = u.tags
      .getAllValues("identifier")
      .stream()
      .map(Arg::get)
      .collect(Collectors.toList());
  ...
  private static MagicPathInts getTotalGems(Nation n) {
    ...
    n
      .selectCommanders()
      .filter(u ->
        u.tags.getValue("schoolmage").filter(a -> a.getInt() == 3).isPresent()
      )
      .forEach(u -> {
        ...
        if (unitpower == null) unitpower = u.getMagicPicks();
