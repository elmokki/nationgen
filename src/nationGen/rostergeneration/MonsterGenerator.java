...
public class MonsterGenerator {
  ...
  public Unit generateMonster() {
    ...
    List<ShapeShift> poses = ChanceIncHandler.retrieveFilters(
      "monsters",
      "default_monsters",
      assets.monsters,
      null,
    ...
    if (poses.size() == 0) {
      System.out.println(
        "No monsters for " +
        n.races.get(0).name +
        ". Try #monsterchance 0 if this race is not supposed to receive monsters."
      );
      return null;
    }
    ...
    ShapeShift p = chandler
      .handleChanceIncs(templateUnit, poses)
      .getRandom(rand);
