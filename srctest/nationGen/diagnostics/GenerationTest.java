...
public class GenerationTest {
    ...
    for (Race baseRace : assets.races) {
      if (!baseRace.tags.containsName("secondary")) {
        ...
        for (String freeThemeType : baseRace.tags.getAllStrings("freetheme")) {
          List<Theme> freeThemeChoices = ChanceIncHandler.getFiltersWithType(
            freeThemeType,
            possibleThemes
          );
        ...
        for (Map<String, Theme> themeSet : generateCombinations(
          themeChoiceMap
        )) {
          ...
          System.out.print(
            race.visiblename + " (" + race.name + ") " + themeSet + ": "
          );
   ...
   */
  private static <K, V> List<Map<K, V>> generateCombinations(
    Map<K, List<V>> choices
  ) {
